<script setup lang="ts">
  ...
  const names = ref(['','']);
  const stageQuest = ref([0,0]);
  const secretkey = 'mylovezai';
  let state = localStorage.getItem("state");
  if (!state) {
    router.push('/home');
  } else if (state !== "quest_four") {
    router.push(`/${state}`);
  }
  let stateQuest = localStorage.getItem("q4");
  if (stateQuest) {
  ...
  function route(ind: number, force = true) {
    const index = quest_data.bestOf2.findIndex((word: string) => AES.decrypt(word, secretkey).toString(enc.Utf8).toLowerCase()===(names.value[ind].trim() as string).toLowerCase());
    if (force) {
      names.value[ind] = '';
    }
    if (index > -1) {
      stageQuest.value[index] = 1;
      localStorage.setItem("q4", stageQuest.value.toString());
    }
    if (stageQuest.value.every((val: number)=> val === 1)){
      localStorage.setItem("state", "quest_five");
      router.push('/quest_five');
    }
  }
</script>
...
<template>
  <div class='home-container'>
    <div class='top-quest-four-container'></div>
    <div class='middle-quest-head-container'>
      <div class='middle-quest-title'></div>
      <div class='middle-quest-four-container-content'>
        <div class="middle-quest-two-span">
          <span>1. {{AES.decrypt(quest_data.bestOf4[0], secretkey).toString(enc.Utf8)}} + {{AES.decrypt(quest_data.bestOf4[1], secretkey).toString(enc.Utf8)}} =</span>
          <span v-if="stageQuest[0]">{{AES.decrypt(quest_data.bestOf2[0], secretkey).toString(enc.Utf8)}}</span>
        </div>
        ...
        <div class="middle-quest-two-span">
          <span>2. {{AES.decrypt(quest_data.bestOf4[2], secretkey).toString(enc.Utf8)}} + {{AES.decrypt(quest_data.bestOf4[3], secretkey).toString(enc.Utf8)}} =</span>
          <span v-if="stageQuest[1]">{{AES.decrypt(quest_data.bestOf2[1], secretkey).toString(enc.Utf8)}}</span>
        </div>
...
.middle-quest-four-container-content {
  display: flex;
  flex-direction: column;
  gap: 10px;
  padding-left: 50px;
